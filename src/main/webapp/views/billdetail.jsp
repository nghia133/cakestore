...
<body>
	...
	<div id="page" class="page">
		...
		<table class="TableData">
			...
			<c:forEach var="receiptitem" items="${receiptitem}">
				<tr>
					<td><c:out value="${receiptitem.product.name}" /></td>
					<td><c:out value="${receiptitem.product.price}" /></td>
					<td><c:out value="${receiptitem.receiptItemQuantity}" /></td>
					<td><c:out
							value="${receiptitem.product.price*receiptitem.receiptItemQuantity}" /></td>
		...
		<div class="footer-right">
			...
			<div>Total:${receipt.receiptPay}</div>

			<div>
				VAT (10%):
				<c:out value="${receipt.receiptPay * 0.1}" />
			</div>

			<div>
				Pay:
				<c:out value="${receipt.receiptPay + (receipt.receiptPay * 0.1)}" />
			</div>
